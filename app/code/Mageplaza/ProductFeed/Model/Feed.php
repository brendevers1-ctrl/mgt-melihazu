	...
	public function getMatchingProductIds() {
		if ($this->productIds === null) {
			...
			if ($data) {
				...
				$data2 = $this->serializer->unserialize($this->getData('conditions_serialized'));
				$conditions = $data2['conditions'] ?? [];

				$data['conditions']['1'] = array(
					'type' => $data2['type'],
					'aggregator' => $data2['aggregator'],
					'value' => $data2['value'],
					'new_child' => '',
				);
				foreach ($conditions as $key => $value) {
					$data['conditions']['1--' . ($key + 1)] = $value;
					unset($data['conditions']['1--' . ($key + 1)]['is_value_processed']);
				}
